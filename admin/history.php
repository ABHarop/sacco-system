...
<?php include('db.php');?>
<h2 class="page-title">MEMBER LOG</h2>
<form method="POST">
...
<?php 
$q=mysqli_query($con,"SELECT * FROM historytb ");
$rr=mysqli_num_rows($q);
if(!$rr)
{
echo "<h2 style='color:red; padding-left:230px'>No User Log...</h2>";
...
if (isset($_GET['page_no']) && $_GET['page_no']!="") {
  ...
  $offset = ($page_no-1) * $total_records_per_page;
	...
	$result_count = mysqli_query($con,"SELECT COUNT(*) As total_records FROM `historytb`");
	$total_records = mysqli_fetch_array($result_count);
	$total_records = $total_records['total_records'];
  $total_no_of_pages = ceil($total_records / $total_records_per_page);
	$second_last = $total_no_of_pages - 1; // total page minus 1

  $result = mysqli_query($con,"SELECT * FROM `historytb` ORDER BY schedule DESC LIMIT $offset, $total_records_per_page");
  while($row = mysqli_fetch_array($result)){

$q1=mysqli_query($con,"SELECT * FROM membertb WHERE memberId='".$row['memberId']."'");
$r1=mysqli_fetch_assoc($q1);

$q2=mysqli_query($con,"SELECT * FROM historytb WHERE historyId='".$row['historyId']."' ");
