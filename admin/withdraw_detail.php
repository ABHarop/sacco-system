...
<?php include('db.php');?>
<h2 class="page-title">PERSONAL WITHDRAW HISTORY</h2>
<form method="POST">
<?php
...
<?php 
$q=mysqli_query($con,"SELECT * FROM withdrawtb WHERE memberId='".$_GET['memberId']."' ");
$rr=mysqli_num_rows($q);
if(!$rr)
{
echo "<h2 style='color:red; padding-left:230px'>No Withdraw Data Yet...</h2>";
...
if (isset($_GET['page_no']) && $_GET['page_no']!="") {
  ...
  $offset = ($page_no-1) * $total_records_per_page;
	...
	$result_count = mysqli_query($con,"SELECT COUNT(*) As total_records FROM `withdrawtb` ");
	$total_records = mysqli_fetch_array($result_count);
	$total_records = $total_records['total_records'];
  $total_no_of_pages = ceil($total_records / $total_records_per_page);
	$second_last = $total_no_of_pages - 1; // total page minus 1

  $result = mysqli_query($con,"SELECT * FROM `withdrawtb` WHERE memberId='".$_GET['memberId']."' ORDER BY date DESC LIMIT $offset, $total_records_per_page");
...
echo "<td>".$r1['withdrawId']."</td>";
echo "<td>".$r1['date']."</td>";
echo "<td>".$r1['amountWithdrawn']."</td>";
