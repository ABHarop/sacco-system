...
<h2 class="page-title">UPDATE ADMIN DETAILS</h2>
<centre>
...
{  
  ...
  $activity="updated administrator information";
  $query2=mysqli_query($con,"SELECT * FROM admintb WHERE contact='$contact' ")or die(mysqli_error($con));
  $count=mysqli_num_rows($query2);
  $passnew=password_hash($password, PASSWORD_BCRYPT);
  if ($password<>''){
    mysqli_query($con,"UPDATE admintb SET photo='$pic', admin_name='$admin_name',gender='$gender',status='$status', username='$username', pass='$passnew',status='$status' WHERE adminId=' ".$_GET['adminId']."' ");
    ...
    if ($count>0)
    {
      mysqli_query($con,"UPDATE admintb SET photo='$pic', admin_name='$admin_name',gender='$gender', username='$username', status='$status'  WHERE adminId=' ".$_GET['adminId']."' ");
      mysqli_query($con,"INSERT INTO admin_log(adminId,activity,schedule) VALUES('$adminId','$activity','$schedule')")or die(mysqli_error($con));
      $err="<font color='blue'>Saved, contact in use</font>";
    } 
    else {
      if($status=='Active'){
        mysqli_query($con,"UPDATE admintb SET photo='$pic', admin_name='$admin_name',gender='$gender',contact='$contact', username='$username', pass='$passnew', status='$status' WHERE adminId=' ".$_GET['adminId']."' ");
        mysqli_query($con,"INSERT INTO admin_log(adminId,activity,schedule) VALUES('$adminId','$activity','$schedule')")or die(mysqli_error($con));
...
$sql=mysqli_query($con,"SELECT * FROM admintb WHERE adminId='".$_GET['adminId']."'");
...
<div>
<div>
<center><img style="width:17%;height:25%" alt="Admin Photo" src="image/<?php echo $res['photo'];?>">
</center>
</div>
...
<input type="file" style="width:17%;margin-top:4px" class="label-pay" name="image">
</center>
</div>

<div class="other-label">Status:
<input class="label-pay" style="color:white;background-color:black;text-align:center" value="<?php echo $res['status']; ?>" name="status" readonly>
</div>

<div class="other-label">Name:
<input class="label-pay" value="<?php echo $res['admin_name']; ?>" name="admin_name" required>
</div>
...
<button type="button" class="cancelbtn" onClick="location.href='admin.php'" >Back</button>
</div>
</center>
</div>
</form>
